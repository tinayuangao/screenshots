...
export class FirebaseService {
        ...
        switch(childSnapshot.key) {
          case 'sha':
            ...
            this._databaseRef().child(`result/${childValue}`).once('value')
              ...
              this.screenshotResult.result = snapshot.val();
            });
            break;
          case 'travis':
          ...
          case 'results':
            ...
            childSnapshot.forEach((resultSnapshot: firebase.database.DataSnapshot) => {
              ...
              counter++;
              if (counter == childSnapshot.numChildren()) {
                return true;
              }
            });
            break;
  ...
  getGithubStatus() {
    let url = `https://api.github.com/repos/${config.repoSlug}/commits/${this.screenshotResult.sha}/status`;
